      ...
      <section v-if="activeTab === 'Setup'" class="flex-1 p-8 overflow-y-auto">
        ...
        <div v-if="profiles.length" class="mt-10 max-w-5xl">
          <div class="flex items-center gap-3 mb-4">
            <span class="px-2 py-0.5 bg-[#66BB6A]/10 text-[#66BB6A] text-[10px] font-mono border border-[#66BB6A]/20">02</span>
            <h3 class="font-mono text-xs text-[#6B6B6B] uppercase tracking-widest">Agent Profiles ({{ profiles.length }})</h3>
            <span class="text-[9px] font-mono text-[#66BB6A]">✓ Complete</span>
          </div>
          <div class="grid grid-cols-1 gap-3 max-h-[600px] overflow-y-auto pr-2">
            <div v-for="(p, idx) in profiles" :key="p.agent_id ?? idx"
              class="p-4 bg-[#FFFFFF] border border-[#E0DDD5]/30 hover:border-[#66BB6A]/30 transition-colors">
              <!-- Row 1: Identity -->
              <div class="flex items-start justify-between mb-2">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 bg-[#E0DDD5] flex items-center justify-center font-mono text-xs text-[#66BB6A] flex-shrink-0">{{ p.agent_id ?? idx }}</div>
                  <div>
                    <span class="text-sm font-bold text-[#2D2D2D]">{{ p.realname || p.name }}</span>
                    <span class="ml-2 text-[10px] font-mono text-[#6B6B6B]">@{{ p.username || p.handle }}</span>
                    <div class="flex items-center gap-1.5 mt-0.5">
                      <span class="px-2 py-0.5 text-[9px] font-mono uppercase bg-[#66BB6A]/10 text-[#66BB6A] border border-[#66BB6A]/20">{{ p.entity_type || p.role }}</span>
                      <span v-if="p.stance_label" class="text-[9px] font-mono"
                        :class="p.stance_label === 'supportive' ? 'text-[#66BB6A]' : p.stance_label === 'opposing' ? 'text-[#FF8A80]' : 'text-[#FFE066]'">
                        {{ p.stance_label }}
                      </span>
                    </div>
                  </div>
                ...
                <!-- Follower count -->
                <div v-if="p.follower_count" class="text-right">
                  <span class="text-[9px] font-mono text-[#6B6B6B] block">followers</span>
                  <span class="text-sm font-mono text-[#B5E8F0]">{{ p.follower_count >= 1000 ? (p.follower_count / 1000).toFixed(1) + 'K' : p.follower_count }}</span>
                </div>
              </div>

              <!-- Row 2: Demographics -->
              <div class="pl-11 flex flex-wrap items-center gap-3 mb-2">
                <span v-if="p.age" class="text-[10px] font-mono text-[#6B6B6B]">🎂 {{ p.age }} tuổi</span>
                <span v-if="p.gender && p.gender !== 'other'" class="text-[10px] font-mono text-[#6B6B6B]">{{ p.gender === 'male' ? '♂' : '♀' }} {{ p.gender }}</span>
                <span v-if="p.gender === 'other'" class="text-[10px] font-mono text-[#6B6B6B]">🏢 organization</span>
                <span v-if="p.mbti" class="px-1.5 py-0.5 text-[9px] font-mono bg-[#D5C4F7]/10 text-[#D5C4F7] border border-[#D5C4F7]/20">{{ p.mbti }}</span>
                <span v-if="p.country" class="text-[10px] font-mono text-[#6B6B6B]">📍 {{ p.country }}</span>
                <span v-if="p.profession" class="text-[10px] font-mono text-[#FFE066]">💼 {{ p.profession }}</span>
              </div>

              <!-- Row 3: Bio -->
              <p v-if="p.bio" class="text-xs text-[#2D2D2D] leading-relaxed mb-2 pl-11 font-medium">{{ p.bio }}</p>

              <!-- Row 4: Persona (truncated, expandable) -->
              <div v-if="p.user_char || p.persona" class="pl-11 mb-3">
                ...
                <button v-if="(p.user_char || p.persona || '').length > 200"
                  ...
                  class="text-[10px] font-mono text-[#66BB6A] mt-1 hover:underline">
                  {{ expandedProfiles.has(p.agent_id) ? '▲ Thu gọn' : '▼ Xem đầy đủ' }}
                </button>
              </div>

              <!-- Row 5: Topics -->
              <div v-if="p.topics?.length" class="pl-11 mb-3 flex flex-wrap gap-1.5">
                <span v-for="t in p.topics.slice(0, 5)" :key="t"
                  class="px-2 py-0.5 text-[9px] font-mono bg-[#E0DDD5] text-[#6B6B6B] border border-[#E0DDD5]">#{{ t }}</span>
              </div>

              <!-- Row 5b: Initial Interests -->
              <div v-if="getInitialInterests(p).length" class="pl-11 mb-3">
                <span class="text-[9px] font-mono text-[#6B6B6B] uppercase block mb-1">Sở thích ban đầu</span>
                <div class="flex flex-wrap gap-1.5">
                  <span v-for="int in getInitialInterests(p)" :key="int"
                    class="px-2 py-0.5 text-[9px] font-mono bg-[#B5E8F0]/15 text-[#0097A7] border border-[#B5E8F0]/30">📌 {{ int }}</span>
                </div>
              </div>

              <!-- Row 6: Behavior stats -->
              <div class="pl-11 grid grid-cols-6 gap-3 text-center">
                <div>
                  <span class="text-[9px] font-mono text-[#6B6B6B] uppercase block">Posts</span>
                  <span class="text-sm font-mono text-[#2D2D2D]">{{ p.posting_probability != null ? p.posting_probability.toFixed(1) : '—' }}</span>
                </div>
                <div>
                  <span class="text-[9px] font-mono text-[#6B6B6B] uppercase block">Comments</span>
                  <span class="text-sm font-mono text-[#2D2D2D]">{{ p.comments_per_time != null ? p.comments_per_time.toFixed(1) : '—' }}</span>
                </div>
                <div>
                  <span class="text-[9px] font-mono text-[#6B6B6B] uppercase block">Delay</span>
                  <span class="text-sm font-mono text-[#2D2D2D]">{{ p.response_delay_label || '—' }}</span>
                </div>
                <div>
                  <span class="text-[9px] font-mono text-[#6B6B6B] uppercase block">Activity</span>
                  <span class="text-sm font-mono text-[#D5C4F7]">{{ p.activity_level != null ? (p.activity_level * 100).toFixed(0) + '%' : '—' }}</span>
                </div>
                <div>
                  <span class="text-[9px] font-mono text-[#6B6B6B] uppercase block">Sentiment</span>
                  <span class="text-sm font-mono" :class="p.sentiment_bias > 0.1 ? 'text-[#66BB6A]' : p.sentiment_bias < -0.1 ? 'text-[#FF8A80]' : 'text-[#FFE066]'">
                    {{ p.sentiment_bias != null ? (p.sentiment_bias > 0 ? '+' : '') + p.sentiment_bias.toFixed(1) : '—' }}
                  </span>
                </div>
                <div>
                  <span class="text-[9px] font-mono text-[#6B6B6B] uppercase block">Influence</span>
                  <span class="text-sm font-mono text-[#B5E8F0]">{{ p.influence_score != null ? p.influence_score.toFixed(1) : '—' }}</span>
                </div>
              </div>

              <!-- Row 7: Cognitive Traits (from MBTI) -->
              <div v-if="p.mbti" class="pl-11 mt-3 pt-3 border-t border-[#E0DDD5]/50">
                <div class="flex items-center gap-2 mb-2">
                  <span class="material-symbols-outlined text-xs text-[#9B59B6]">neurology</span>
                  <span class="text-[9px] font-mono text-[#6B6B6B] uppercase">Nhận thức tính cách</span>
                </div>
                <div class="grid grid-cols-4 gap-3">
                  <div class="text-center">
                    <span class="text-[8px] font-mono text-[#6B6B6B] block">Bảo thủ</span>
                    <span class="text-xs font-mono font-bold" :class="getCogTraits(p.mbti).conviction >= 0.7 ? 'text-[#FF8A80]' : getCogTraits(p.mbti).conviction <= 0.4 ? 'text-[#66BB6A]' : 'text-[#FFE066]'">{{ getCogTraits(p.mbti).conviction.toFixed(2) }}</span>
                  </div>
                  <div class="text-center">
                    <span class="text-[8px] font-mono text-[#6B6B6B] block">Hay quên</span>
                    <span class="text-xs font-mono font-bold" :class="getCogTraits(p.mbti).forgetfulness >= 0.2 ? 'text-[#FF8A80]' : getCogTraits(p.mbti).forgetfulness <= 0.1 ? 'text-[#66BB6A]' : 'text-[#FFE066]'">{{ getCogTraits(p.mbti).forgetfulness.toFixed(2) }}</span>
                  </div>
                  <div class="text-center">
                    <span class="text-[8px] font-mono text-[#6B6B6B] block">Tò mò</span>
                    <span class="text-xs font-mono font-bold" :class="getCogTraits(p.mbti).curiosity >= 0.4 ? 'text-[#66BB6A]' : getCogTraits(p.mbti).curiosity <= 0.2 ? 'text-[#FF8A80]' : 'text-[#FFE066]'">{{ getCogTraits(p.mbti).curiosity.toFixed(2) }}</span>
                  </div>
                  <div class="text-center">
                    <span class="text-[8px] font-mono text-[#6B6B6B] block">Dễ ảnh hưởng</span>
                    <span class="text-xs font-mono font-bold" :class="getCogTraits(p.mbti).impressionability >= 0.2 ? 'text-[#FF8A80]' : getCogTraits(p.mbti).impressionability <= 0.1 ? 'text-[#66BB6A]' : 'text-[#FFE066]'">{{ getCogTraits(p.mbti).impressionability.toFixed(2) }}</span>
                  </div>
                </div>
              </div>
            </div>
...
async function prepareSimulation() {
  ...
  try {
    ...
    const res = await simApi.prepare(store.campaignId, numAgents.value, numRounds.value, store.groupId, toggles)
