...
export const graphApi = {
  // Build pipeline chạy LLM analyze per-section + Graphiti add_episode (mỗi episode ~20s)
  // với campaign trung bình 5-10 section → timeout cần ≥ 5 phút.
  build: (payload) => api.post('/graph/build', payload, { timeout: 600000 }),
...
export const simApi = {
  prepare: (campaignId, numAgents = 10, numRounds = 3, groupId = '', cognitiveToggles = {}, crisisEvents = []) =>
  ...
  injectCrisis: (simId, crisisData) => api.post(`/sim/${simId}/inject-crisis`, crisisData),
  crisisLog: (simId) => api.get(`/sim/${simId}/crisis-log`),
}
