...
async function buildGraph() {
  ...
  // Build succeeded — load graph data separately so load errors don't mask build success
  building.value = false
  try {
    // Small delay to let FalkorDB finalize indexing after Graphiti add_episode
    await new Promise(r => setTimeout(r, 2000))
    await loadAvailableGraphs()
    await loadGraphData()
  } catch (e) {
    console.warn('[GraphView] Graph built successfully but failed to load data:', e)
    error.value = 'Graph built thành công! Đang tải dữ liệu... Thử nhấn lại nút refresh.'
  }
}
