...
## FalkorDB graph memory (current status)

⛔ **Graph cognition disabled hiện tại** ([apps/simulation/agent_cognition.py:GraphCognitiveHelper._DISABLED=True](../apps/simulation/agent_cognition.py)). `graphiti_core.driver.falkordb_driver.FalkorDriver` không tồn tại trên bất kỳ version PyPI nào → import fail → wrapper return empty.

**Theoretically khi enable**: Interview có thể query social context:
- "Bạn follow ai trong sim?" → query `(SimAgent)-[:KNOWS]-(SimAgent)` trong sim graph
- "Bạn đã tương tác với ai nhiều nhất?" → aggregate posts/comments edges

Data giữ nguyên sau sim COMPLETED — design intent cho post-simulation analytics. Refactor sang direct Cypher (raw FalkorDB client) là pending; xem `05_simulation_loop.md` section "Graph cognition — current state" cho refactor plan.
...
- **`UnboundLocalError` legacy bug** ([apps/simulation/api/interview.py](../apps/simulation/api/interview.py)): trước đây `chat_with_agent` shadow module-level `_sim_dir()` function bằng local string variable cùng tên ở nhánh enrichment → Python compile body với `_sim_dir` là local → call ở đầu hàm (trước nhánh enrich) UnboundLocalError. Fix: rename local `_sim_dir_path`. Pattern này cần tránh ở mọi function khác.
- **sim_dir resolution**: `_sim_meta()` query meta.db cho sim row (`sid, sim_dir, oasis_db_path, config_path, profiles_path`). Phase 5 path là `data/campaigns/<cid>/sims/<sid>/`.
